...
class XMLGenerator {
    ...
    static neDir = "${conf.inputConfig.input.files.path}/${conf.inputConfig.input.files.named.entities}"

    synchronized generateXML(dirName) {
        def inputDir = input.readInputDirectory(dirName)
        inputDir.each { inputData ->
            ...
            def ne = ner.loadNamedEntities(neDir)
                                            ...
                                            NamedEntities() {
                                                ne_in_sentences.eachWithIndex { named_entity, idx ->
                                                    if (named_entity.entity_sentence.id == stc_index + 1) {
                                                        NamedEntity(id: "${idx + 1}",
                                                                named_entity.entityName)
                                                    }
                                                }
                                            }
                                            Words() {
        ...
        input.renameProcessedFiles(dirName, inputDir)
    }
